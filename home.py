...
@app.route('/start')
def start():
    return "Hello, my name is Tracks. I will help you register your fleet by asking you some questions."
...
def askTruck(truckQuestionNumber, fleetId):
    ...
    return truckQuestions[truckQuestionNumber]["question"]
    ...
    if 'CARDINAL' in list(groupNumberEntities.keys()):
        ...
        for i in range(int(groupNumber)):
            for j in range(4): #number of truck questions
                question = {}
                questionTxt = random.choice(list(truckQuestions[str(j)]["questions"]))
                questionTag = truckQuestions[str(j)]["tag"]
                if questionTag == "trucks_in_group":
                    questionTxt += str(i+1) + "?"
                question["tag"] = questionTag
                question["entity"] = truckQuestions[str(j)]["entity"]
                question["question"] = questionTxt
                
                allQuestions[str(count)] = question
                count += 1
